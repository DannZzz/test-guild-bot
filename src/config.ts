...
export const ERROR_EMOJI: EmojiResolvable = process.env.LEVEL_EMOJI || "❌"; // Смайл при ошибок
export const SUCCESS_EMOJI: EmojiResolvable = process.env.LEVEL_EMOJI || "✅"; // Смайл при удачных действий
...
export const BOX_VISUAL_TEXT = {
    atBoost(time: string) {
        return stripIndents`Вы получили **Буст Профиля** на ${time}`
    },
    atXp(money: string) {
        return stripIndents`Вы получили ${money}.`
    }
}
