...
import { ONE_MINUTE_VOICE } from "../config";
    ...
    async run({ client, ctx }) {
        ...
        if (newState.channel) {
            ...
            if (ended) {
                ...
                const minutes = DateTime.getTimeData(Date.now() - ended.getTime(), 0).minutes
                const points = minutes && minutes > 0 ? Math.round(minutes * ONE_MINUTE_VOICE) : 0;
                // console.log(points)
