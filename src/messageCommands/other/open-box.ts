import ms from "ms";
import { BOX_RANDOM_REWARD, CURRENCY, DEFAULT_BOOST_AMOUNT } from "../../config";
...
export default new MessageCommand ({
    ...
    async run ({msg, methods, client}) {
        ...
        if (reward.type === "boost") {
            ...
            return Embed(msg).setSuccess(`Вы получили **Буст Профиля** на ${DateTime.formatTime(DateTime.getTimeData(time, 0))}`).send();
        } else if (reward.type === "xp") {
            const xp = client.util.random(reward.min, reward.max);

            await givePoints(id, xp, true);
            return Embed(msg).setSuccess(`Вы получили ${CURRENCY.main} ${client.util.formatNumber(xp)}.`).send();
        }
